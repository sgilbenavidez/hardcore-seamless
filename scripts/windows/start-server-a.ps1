<#
.SYNOPSIS
    Starts Server A (Fabric, Minecraft 1.20.1, Java 17) in the foreground.
...
Write-Host "=== start-server-a.ps1 ===" -ForegroundColor Cyan
...
if ($listener) {
    ...
    throw "Port $port is already in use by PID $procId. Refusing to start a second Server A instance. Verify with scripts\windows\show-processes.ps1 before proceeding."
}
Write-Host "Port $port is free."

# 3. Working directory.
$serverDir = "E:\minecraft-hardcore\server-a"
...
try {
    & $javaExe "-Dhardcore.backendId=server-a" "-Xms$Xms" "-Xmx$Xmx" -jar "fabric-server-launch.jar" nogui
} finally {
    Pop-Location
}
